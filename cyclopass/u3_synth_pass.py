...
from bqskit.ir.gates.constant import HGate, TGate, SGate, XGate, YGate, ZGate
import cyclosynth

from bqskit.qis.unitary.unitarymatrix import UnitaryMatrix


def cyclo_decompose(un: np.ndarray, epsilon: float) -> Circuit:
    """
    ...
    async def run(self, circuit: Circuit, data: PassData) -> None:
        ...
        for cycle, op in circuit.operations_with_cycles():
            if isinstance(op.gate, U3Gate):
                ...
                un = op.get_unitary()
                u3_circuit = cyclo_decompose(un.numpy, error_per_gate)
