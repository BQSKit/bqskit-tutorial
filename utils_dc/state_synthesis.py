...
from utils_dc.dc_instantiation_state import cost_function_modular
from utils_dc.dc_instantiation_state import state_infidelity_grad_modular
...
def optimize_params(start):
    ...
    num_params, initial_layer, branch_circuits, target_states, ancillas = start
    np.random.seed(None)
    ...
    result = minimize(cost_function_modular, initial_params,
                      args=(initial_layer, branch_circuits, target_states, ancillas),
                      method='BFGS', jac=state_infidelity_grad_modular)
    cost = cost_function_modular(result.x, initial_layer, branch_circuits, target_states, ancillas)
    return cost, result.x


def optimize_successor(args):
    successor, branch_circuits, target_states, multi_starts, ancillas = args
    ...
    for _ in range(multi_starts):
        ...
        result = minimize(cost_function_modular, initial_params,
                          args=(successor, branch_circuits, target_states, ancillas),
                          method='BFGS', jac=state_infidelity_grad_modular)
        cost = cost_function_modular(result.x, successor, branch_circuits, target_states, ancillas)
...
class StateSynthesisPass():
    ...
    def synthesize(
            self,
            target_states: list[StateVector],
            ancillas: list,
        ...
        if coupling_graph is None:
            coupling_graph = [(i, j) for i in range(target_states[0].num_qudits) for j in
                              range(i + 1, target_states[0].num_qudits)]
        # Begin the search with an initial layer
        frontier = Frontier(target_states, self.branch_circuits, ancillas)

        if initial_layer is None:
            initial_layer = layer_gen.gen_initial_layer(target_states[0])

        num_params = initial_layer.num_params
        for circuit in self.branch_circuits:
            num_params += circuit.num_params
        multi_starts = 20
        starts = [(num_params, initial_layer, self.branch_circuits, target_states, ancillas) for _ in
                  range(multi_starts)]
        ...
        while not frontier.empty():
            ...
            args_list = [(successor, self.branch_circuits, target_states, multi_starts, ancillas) for successor in
                         successors]
            ...
            for successor, best_params in zip(successors, successor_best_params):
                dist = cost_function_modular(best_params, successor, self.branch_circuits, target_states, ancillas)
