...
class SimpleLayerGenerator(LayerGenerator):
    ...
    def gen_initial_layer(
        ...
        if isinstance(target, int):
            init_circuit = Circuit(target)
        else:
            init_circuit = Circuit(target.num_qudits)
    ...
    def gen_successors(self, circuit: Circuit, coupling_graph: list) -> list[Circuit]:
        """
        ...
        # Generate successors
        successors = []
        original_depth = circuit.multi_qudit_depth
        for edge in coupling_graph:
            ...
            if successor.multi_qudit_depth == original_depth:
